...
class Hyperparameters:
    ...
    shared_qkv_layers = int(os.environ.get("SHARED_QKV_LAYERS", 0))
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        rope_base: float,
        qk_gain_init: float,
        shared_qkv: "SharedQKV" | None = None,
    ):
        super().__init__()
        if dim % num_heads != 0:
        ...
        self.c_k = None
        self.c_v = None
        self._shared_qkv = None
        if shared_qkv is None:
            self.c_q = CastedLinear(dim, dim, bias=False)
            self.c_k = CastedLinear(dim, kv_dim, bias=False)
            self.c_v = CastedLinear(dim, kv_dim, bias=False)
        else:
            self.__dict__["_shared_qkv"] = shared_qkv
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self._shared_qkv is None:
            q = self.c_q(x).reshape(bsz, seqlen, self.num_heads, self.head_dim)
            k = self.c_k(x).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim)
            v = self.c_v(x).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim)
        else:
            q, k, v = self._shared_qkv(x)
...
class MLP(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        gate = torch.nn.functional.silu(self.fc_gate(x))
        up = self.fc_up(x)
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init, shared_qkv)
...
class GPT(nn.Module):
    def __init__(
        ...
        self.shared_qkv = SharedQKV(model_dim, kv_dim) if shared_qkv_layers > 0 else None
        self.shared_qkv_layers = shared_qkv_layers
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    rope_base,
                    qk_gain_init,
                    shared_qkv=self.shared_qkv if i < shared_qkv_layers else None,
                )
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        shared_qkv_layers=args.shared_qkv_layers,
    ).to(device).bfloat16()
    ...
    if base_model.shared_qkv is not None:
        matrix_params.extend([p for p in base_model.shared_qkv.parameters() if p.ndim == 2])
    ...
    log0(f"shared_qkv_layers:{args.shared_qkv_layers}")
    log0(
